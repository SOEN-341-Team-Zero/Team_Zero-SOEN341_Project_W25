...
public class CreateControllerTests
{
    ...
    private readonly CreateController _controller;
    ...
    public CreateControllerTests() {
        ...
        _controller = new CreateController(_context);
    }

    [Fact]
    public async Task TeamCreation_ReturnsStatusCode201() {
        // Arrange
        ...
        // Act
        var objectResult = (await _controller.TeamCreation(new TeamCreationRequest {team_name = "Test"})) as ObjectResult;
        ...
        Assert.Equal("Team created successfully", (objectResult.Value as dynamic).message);
    }

    [Fact]
    public async Task TeamCreation_ReturnsBadRequest_WhenUserIsNotFound() {
        // Arrange
        _controller.ControllerContext = new ControllerContext {HttpContext = new DefaultHttpContext {User = new ClaimsPrincipal(new ClaimsIdentity([new Claim(ClaimTypes.NameIdentifier, "John")], "mock"))}};
        await _context.SaveChangesAsync();

        // Act
        var result = (await _controller.TeamCreation(new TeamCreationRequest {team_name = "Test"})) as BadRequestObjectResult;
